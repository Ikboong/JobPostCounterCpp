...
권장 운영 구조는 Naver Cloud Functions가 한국 리전에서 잡코리아를 조회한 뒤, GitHub의 `.github/workflows/record-jobkorea-count.yml`을 `workflow_dispatch`로 호출하는 방식입니다.

- Naver Cloud Functions가 잡코리아 공고 수를 측정합니다.
- GitHub Actions는 외부에서 받은 측정값을 CSV/XLSX에 기록합니다.
- 같은 날짜에 workflow가 여러 번 실행돼도 `DateKST` 기준 하루 1행만 유지합니다.
...
Naver Cloud Functions용 코드는 `naver-cloud-functions/jobkorea_dispatcher.js`에 있습니다. Cron Trigger는 09:00, 09:10, 09:30 KST에 등록합니다. GitHub Actions가 잡코리아에 직접 접근하는 `.github/workflows/daily-job-post-counter.yml`은 수동 백업용으로 남겨둡니다.
...
웹 차트와 XLSX의 `WeeklyOHLC`/`WeeklyCandles`는 `JobKoreaStatus`가 `ok`이고 count가 숫자인 행만 반영합니다.
